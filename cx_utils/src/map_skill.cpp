 ...
 * Param: enter-field: drive_into_field{team=?(team-color)S,
 * wait=?(r|/R-1/0.0/|/R-2/3.0/|/R-3/6.0/)f} Examples: (enter_field R-1 CYAN) ->
 * drive_into_field{team="CYAN", wait=0.000000} (enter_field R-2 CYAN) ->
 * drive_into_field{team="CYAN", wait=3.000000} Note: the chaining of regular
 * expressions to fill in a parameter based on the specific value of another
 * parameter. You can also see that arguments can be referenced more than once.
 *
 ...
 */
void ActionSkillMapping::add_mapping(const std::string &action_name,
 ...
 */
bool ActionSkillMapping::has_mapping(const std::string &action_name) const {
 ...
 * message.
 * @return The skill string of the mapped action, or an empty string in case of
 * an error.
 */
std::string ActionSkillMapping::map_skill(
      ...
      if (p.first == m[1].str()) {
          ...
          for (const auto &r : rlst) {
            if (r.size() > 2 && r[0] == '/' && r[r.size() - 1] == '/') {
              ...
              if (slash_pos != std::string::npos &&
                ...
                std::string r_repl =
                    r.substr(slash_pos + 1, (r.size() - slash_pos - 2));
                std::regex user_regex(r_match, std::regex::ECMAScript |
                                                   std::regex::icase);
                value = std::regex_replace(value, user_regex, r_repl);
              } else {
                messages.insert(std::make_pair(
                    "WARN", " regex '" + r + "' missing mid slash, ignoring"));
              }
            } else {
              messages.insert(std::make_pair(
                  "WARN",
                  "regex '" + r + "' missing start/end slashes, ignoring"));
            }
          }
        }
        ...
        case 'i':
          try {
            ...
            messages.insert(
                std::make_pair("ERROR", "Failed to convert '" + value +
                                            "' to integer: " + e.what()));
            return "";
          }
          break;
