...
protected:
  ...
  vector<string> clips_files = {"../../../clips/clips/utils.clp", "../plan.clp",
                                "../domain.clp"};
};
...
TEST_F(BlocksworldDomainTest, PreconditionsAreSatisfiedTest) {
  ...
  env.run();
  EXPECT_TRUE(
      has_fact("((?p domain-precondition))",
...
TEST_F(DomainTest, Typing) {
  ...
  EXPECT_TRUE(
      has_ordered_fact("domain-obj-is-of-type", {"c1", "moveable-obj"}));
...
TEST_F(DomainTest, PreconditionWithUnknownParameter) {
  ...
  env.run();
  EXPECT_TRUE(
      has_fact("((?e domain-error))", "(eq ?e:error-type unknown-parameter)"));
}
...
TEST_F(ConditionalSayDomainTest,
       DISABLED_DoNotApplyCondEffectIfCondDoesNotHold) {
  env.reset();
  env.assert_fact("(plan-action"
                  " (id 1)"
                  " (goal-id g0) (plan-id p0)"
