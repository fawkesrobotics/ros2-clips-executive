...
#include "cx_executive_feature/executive_feature.hpp"
...
namespace cx {
ExecutiveFeature::ExecutiveFeature() {}
...
void ExecutiveFeature::initialize() {
  ...
  feature_path_ =
      ament_index_cpp::get_package_share_directory("cx_executive_feature");

  cx::cx_utils::declare_parameter_if_not_declared(
      node, feature_name_ + ".publish_on_refresh",
      rclcpp::ParameterValue(false));
  cx::cx_utils::declare_parameter_if_not_declared(
      node, feature_name_ + ".assert_time", rclcpp::ParameterValue(true));
  cx::cx_utils::declare_parameter_if_not_declared(
      node, feature_name_ + ".refresh_rate", rclcpp::ParameterValue(10));
  node->get_parameter(feature_name_ + ".publish_on_refresh",
                      publish_on_refresh_);
  node->get_parameter(feature_name_ + ".assert_time", assert_time_);
  node->get_parameter(feature_name_ + ".refresh_rate", refresh_rate_);
...
std::shared_ptr<rclcpp_lifecycle::LifecycleNode> ExecutiveFeature::get_node() {
  return parent_.lock();
}

bool ExecutiveFeature::clips_env_init(LockSharedPtr<clips::Environment> &env) {
  if (publish_on_refresh_) {
  ...
  }
  {
    ...
    clips::AddUDF(
        ...
        [](clips::Environment *env, clips::UDFContext *udfc,
           clips::UDFValue *out) {
          ExecutiveFeature *instance =
              static_cast<ExecutiveFeature *>(udfc->context);
          double currentTime =
    ...
    std::vector<std::string> files{feature_path_ +
                                   "/clips/cx_executive_feature/time.clp"};
...
bool ExecutiveFeature::clips_env_destroyed(
...
PLUGINLIB_EXPORT_CLASS(cx::ExecutiveFeature, cx::ClipsFeature)
