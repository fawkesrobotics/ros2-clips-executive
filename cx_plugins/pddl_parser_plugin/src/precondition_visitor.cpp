 ...
 */
vector<string> PreconditionToCLIPSFactVisitor::operator()(Atom &a) const {
 ...
 */
vector<string> PreconditionToCLIPSFactVisitor::operator()(Predicate &p) const {
  vector<string> res;
  ...
  if (p.function == "and" || p.function == "not" || p.function == "or") {
    ...
    for (Expression &sub : p.arguments) {
      vector<string> args = boost::apply_visitor(
          PreconditionToCLIPSFactVisitor(name, sub_counter++), sub.expression);
    ...
    for (auto &p : p.arguments) {
      vector<string> p_strings = boost::apply_visitor(
          PreconditionToCLIPSFactVisitor(name, 0), p.expression);
      if (p_strings.size() != 1) {
        throw PddlParserException(
